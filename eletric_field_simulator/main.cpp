...
int main(){
    ...
    interface_.font24 = al_load_font("fonts/fonte.ttf", 24, 0);
    ...
    interface_.initInterface(widht, height);
    ...
    interface_.triangle = al_create_bitmap(9,7);
    interface_.arrow = al_create_bitmap(10,25);
    al_set_target_bitmap(interface_.triangle);
    al_draw_filled_triangle(0, 3.5, 9, 0, 9, 7, al_map_rgb(140,156,172));

    al_set_target_bitmap(interface_.arrow);
    ...
    while(!exit){
        ...
        while(!reset){
            ...
            if(event.type == ALLEGRO_EVENT_TIMER){
                ...
                interface_.drawGrid();
                interface_.drawFieldLines(&a, &p);
                interface_.drawCharges(mouse, &p);

                if(eletroMeterActived)
                    interface_.drawElectroMeter(eletricFieldMousePos);

                interface_.drawbgInterface();
                interface_.drawButtons(b, mouse);

                if(eletroMeterActived){
                    al_draw_scaled_bitmap(interface_.eletromagMeter, 0, 0, 64, 64, mouse.x-16, mouse.y-16, 32,32, 0);
                }
                else if(insertPosCharge){
                    al_draw_scaled_bitmap(interface_.posCharge, 0, 0, 64, 64, mouse.x-16, mouse.y-16, 32,32, 0);
                }
                else if(insertNegCharge){
                    al_draw_scaled_bitmap(interface_.negCharge, 0, 0, 64, 64, mouse.x-16, mouse.y-16, 32,32, 0);
